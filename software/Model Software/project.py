...
# Precondition: int to be size of batch,
# variables that may not be set:
# (These will be used if a model is loaded, which is done if test = 0)
# 'test =' flag for testing or loading memory
# 'load_file =' file to load the models from
# 'validate_file_in =' file path for validation data to come in
# 'validate_file_out = ' file path for sample output 
#
# Postcondition: if testing model is created, and saved in object. if not, model is loaded, validated, and it output is saved
...
# Postcondition: Creates a generator model, which gets an image input based on the shape given previously
# , and an image output
...
# Trains model
# Precondition: path file for testing, validation data path, path file to save sample, file path
# to save results, int number of epochs model is trained in, interval number for images to be printed
...
# Postcondition: Images saved at the given path
...
# Postcondition: Mode saved at path.
